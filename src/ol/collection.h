...
#ifndef OL_COLLECTION_H
#define OL_COLLECTION_H
...
class OLQT_EXPORT Collection : public ol::Object {
	Q_OBJECT
private:
	std::vector<QVariant> array_;
	bool unique_;
...
signals:
	void Event(CollectionEventType::_enum event_type, T data);

public:
	//ol.Collection = function(opt_array, opt_options) {
	Collection(std::vector<T> opt_array = std::vector<T>(), CollectionOptions opt_options = CollectionOptions())
		:array_(opt_array), unique_(false)
	{
		//
		//	ol.Object.call(this);
		//
		...
		if (opt_options.unique.has_value())
			unique_ = opt_options.unique.value();
		//
		//	/**
		//	* @private
		//	* @type {!Array.<T>}
	...
	std::vector<QVariant> const &getArray() const {
		return array_;
	}
	...
	* @api
	*/
	QVariant removeAt (int index) {
		auto prev = array_[index];
		array_.erase(array_.begin() + index);
		// TODO: updateLength_();
		//this.dispatchEvent(
		//	new ol.Collection.Event(ol.CollectionEventType.REMOVE, prev));

		emit Event(ol::CollectionEventType::REMOVE, prev);
